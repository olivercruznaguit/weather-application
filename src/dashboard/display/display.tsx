...
const Display: React.FC<DisplayProps> = ({
	...
	const getWeatherIcon = (weather: string | undefined) => {
		...
		if (weather?.includes("snowy")) {
			return faSnowflake;
		}

		if (weather?.includes("stormy")) {
			return faBolt;
		}
	...
	return (
		<div className="w-full h-full place-items-center flex place-content-center">
			<div className="relative h-full w-1/2">
				<div className={`bg-slate-300 absolute -left-14 top-28 w-96 h-96 filter blur-2xl rounded-full mix-blend-overlay`}></div>
				<div className={`${getWeatherBackground(weather)[0]} absolute right-0 bottom-0 w-96 h-96 filter blur-2xl  rounded-full mix-blend-overlay`}></div>
				<div className={`${getWeatherBackground(weather)[2]} absolute left-0 top-1/4 w-[500px] h-[500px] filter blur-2xl rounded-full mix-blend-overlay`}></div>
			</div>
			<div className="absolute top-0 bottom-0 left-0 right-0 flex items-center justify-center">
				<div className="max-w-lg p-10 rounded-3xl shadow-2xl ">
					...
					<div className="py-5 px-20">
						...
						<h1 className="text-3xl capitalize  text-white">{weather}</h1>
					</div>
				</div>
			</div>
		</div>
